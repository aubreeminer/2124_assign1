...
functionRuntimes timeAlgorithm( char *szName, int iNumRepeats, int iNumTestCaseSizes, int arrTestSizes[], void (*f)(FILE *) )
{
  ...
  strcpy(fRT.szName, szName); // not using this, but if it gives error uncomment */
...
for(i = 0; i < iNumTestCaseSizes; i++)
  ...
  fRT.arrRuntimes =(double**)malloc(iNumTestCaseSizes * sizeof(double*)) ; /* replace NULL with your code all of this was null */

  
      fRT.szName =(char*) malloc((strlen(szName) + 1));
...
void computeAvg( functionRuntimes fRT )
{
  ...
  /*while(i++< fRT.iNumTestCaseSizes){
    while(j++<fRT.iNumRepeats){
      sum += fRT.arrRuntimes[1][j];
//   i++;
      }
   ...
   // j++;
        ...
        //iterate through inumtestcs and inumr
  ...
  {	
    ...
    {	
		...
		fRT.arrAvg[i] = sum / fRT.iNumTestCaseSizes;
		sum = 0;
	} 
...
int j;
	...
	printf("%s", fRT.szName); //i believe this is mystery 1, 2 , 3
  printf("\n"); 
	printf("Test size"); 
  //test size will be preinted on a  new line
	printf("  ");
	//for(i = 0; i < 14; i++); this is wrong
  for(i =0;i<fRT.iNumRepeats; i++)
	...
	for(i = 0; i < fRT.iNumTestCaseSizes; i++) /*double decker array to print each bit of data from runtime array */
  {	
...
i = 0;
  ...
  for( i=0; i< fRT.iNumTestCaseSizes; i++)  //iterate thrpugh test case sizes 
    free(fRT.arrRuntimes[i]); // all of the instances that exist within the size of test case (inumtcs)
